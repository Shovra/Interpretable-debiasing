import argparse
from debias_models.energy_model.train import train
...
parser.add_argument('--reweight', action='store_true',
                    help='whether to reweight the loss with the number of each class')
...
parser.add_argument('--bias_thred', default=0, type=float)
parser.add_argument('--bias_weight', default=0.1, type=float)
parser.add_argument('--debias_method', default='energy', type=str, choices=['energy', 'probability'])
...
abs_string = '_abs' if args.abs else ''

bias_model_string = args.bias_model_path.split("/")[-2].split('_')[-2][-1]
if 'abs' in args.bias_model_path.split("/")[-2]:
    bias_model_string += 'abs'
