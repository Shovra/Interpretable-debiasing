...
from debias_models.energy_model.constants import UNK_TOKEN, PAD_TOKEN
from debias_models.energy_model.plotting import plot_heatmap
...
def filereader(path, label_field="label"):
    ...
    for i, d in enumerate(data):
        try:
            ...
            print(f"Error processing entry {i}, skipping it")
            print(f"Error: {e}")
...
def get_minibatch(data, batch_size=25, shuffle=False):
    ...
    if shuffle:
        ...
        random.shuffle(data[:])  # Prevents in-place shuffling issues
...
def get_args():
    ...
    parser.add_argument('--bias_model_path', type=str, default='model.pt')
